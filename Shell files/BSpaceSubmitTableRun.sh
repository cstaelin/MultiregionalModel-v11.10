...
NETLOGO_MODEL=$1
NETLOGO_EXPERIMENT=$2
NETLOGO_OUTPUTFILE=$3
...
echo The experiment is $NETLOGO_EXPERIMENT.
...
nohup java -server -Xmx1024M -cp netlogo.jar org.nlogo.headless.Main \
  --threads 8 \
  --model "$NETLOGO_MODEL" \
  --experiment "$NETLOGO_EXPERIMENT" \
